...
const AcousticDemo: React.FC = () => {
  ...
  useEffect(() => {
      ...
      const draw = () => {
          if (!isVisible) {
              // Pause loop if not visible to save GPU
    ...
    <section className="bg-[#111] text-white py-20 md:py-32 relative overflow-hidden md:cursor-crosshair">
       ...
       >
           ...
           <div className={`
                ...
                w-48 h-48 md:w-64 md:h-64 rounded-full border border-white/20 
                flex items-center justify-center
                transition-all duration-700 pointer-events-none
                ${isInside ? 'bg-white/5 scale-110 border-white/50' : 'bg-transparent scale-100'}
           `}>
                <span className={`text-xs font-bold uppercase tracking-[0.2em] transition-colors duration-500 ${isInside ? 'text-white' : 'text-white/30'}`}>
